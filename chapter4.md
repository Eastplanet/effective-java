...
추상 클래스는 다중 상속이 불가능하니 추상 클래스가 정의한 타입을 구현하는 클래스는 반드시 하위 클래스가 된다. <br>
기존 클래스에도 손쉽게 새로운 인터페이스를 구현해넣을 수 있다. <br>
또한 디폴트 메소드를 제공할 수 있게 되었다. <br>
반면 추상 클래스는 기존 클래스 위에 끼어넣기가 어렵다. <br>
인터페이스는 믹스인 정의에 안성맞춤이다. 믹스인은 클래스의 주된 타입 외에도 특정 행위를 제공한다고 선언하는 효과이다. (Comparable 등) <br>
인터페이스로는 계층구조가 없는 타입 프레임워크를 만들 수 있다. <br>
가수와 작곡가 인터페이스가 있다면, 없음, 가수, 작곡가, 가수+작곡가를 만들 수 있지만 <br>
추상 클래스로는 2^N개의 클래스나 만들어야 한다.

# 아이템 21. 인터페이스는 구현하는 쪽을 생각해 설계하라

기존 인터페이스에 디폴트 메서드로 새 메서드를 추가하는 일은 꼭 필요한 경우가 아니면 피해야 한다. <br>
추가하려는 디폴트 메서드가 기존 구현체들과 충돌하지는 않을 지 심사숙고해야 함도 당연하다.

# 아이템 22. 인터페이스는 타입을 정의하는 용도로만 사용하라

상수 인터페이스란 static final 필드로만 가득 찬 인터페이스를 말한다.  <br>
상수 인터페이스와 같은 안티패턴은 인터페이스를 잘못 사용한 예다. <br>
